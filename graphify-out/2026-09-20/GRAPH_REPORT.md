# Graph Report - angys_guard  (2026-09-20)

## Corpus Check
- 248 files · ~409,011 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2081 nodes · 3706 edges · 206 communities (106 shown, 46 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 174 edges (avg confidence: 0.9)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `30149413`
...
1. `AppConfig` - 108 edges
2. `LaptopGuard` - 92 edges
...
Cohesion: 0.05
Nodes (25): AudioIntercom, worker(), notify(), Visible near-live voice intercom. Bale Bot API transports voice/audio messages…, inline_keyboard(), LaptopGuard, worker(), worker() (+17 more)

### Community 2 - "FeatureManager"
Cohesion: 0.17
Nodes (9): CallbackHandler, CommandHandler, FeatureConflictError, FeatureManager, Owns deterministic command/callback routes and feature lifecycle., test_duplicate_command_is_rejected(), test_failed_feature_install_rolls_back_partial_routes(), test_overlapping_callback_prefix_is_rejected() (+1 more)
...
Cohesion: 0.05
Nodes (46): BaleApi, Compatibility facade over the consolidated Bale provider adapter. New code must…, check_bot_connectivity_detailed(), Return token-safe connectivity status plus a stable failure category., ProviderAuthError, ProviderConnectionError, ProviderError, ProviderResponseError (+38 more)

### Community 7 - "config.py"
Cohesion: 0.09
Nodes (32): _apply_legacy_env(), _apply_section(), _atomic_write_private(), _coerce_like(), ensure_dirs(), load_config(), _migrate(), Any (+24 more)

### Community 8 - "StopPinStore"
Cohesion: 0.10
Nodes (24): _authorized_safe_exit(), main(), _pid_alive(), Lock when the guard disappears unless it completed owner-authorized exit. The…, watch(), PinCheck, Path, Persistent local shutdown PIN verifier. The plaintext PIN is never stored. The… (+16 more)
...
Cohesion: 0.16
Nodes (21): default_api_base(), build_provider(), get_provider_profile(), ProviderProfile, Documented transport differences for a Telegram-style provider., _ask_valid_token(), _auth_error(), ensure_runtime_configuration() (+13 more)

### Community 14 - "models.py"
Cohesion: 0.11
Nodes (15): ApiConfig, AppsConfig, BotConfig, ChatConfig, CommunicationConfig, MonitorConfig, ScreenConfig, StartupConfig (+7 more)
...
Cohesion: 0.09
Nodes (12): InputMonitor, Detects input *activity*, never stores typed keys or button values. auto mode…, Path, Backward-compatible facade for older diagnostics/tests., ScreenCapture, ScreenCaptureManager, test_first_pynput_mouse_movement_triggers_immediately(), test_single_nonzero_evdev_relative_event_triggers() (+4 more)
...
### Community 19 - "RuntimeStateStore"
Cohesion: 0.15
...
### Community 24 - "guard.py"
Cohesion: 0.10
Nodes (28): play_audio(), Path, record_audio(), cmd_run(), main(), _capability_guidance(), configuration_recovery(), guidance_for_exception() (+20 more)
...
Cohesion: 0.09
Nodes (22): 11.1.0 — 2026-09-12, AI-assisted engineering, AngysGuard brand system, AngysGuard product/platform planning, Architecture baseline, Audio security and communication, Bale / Telegram transport parity, Changelog (+14 more)
...
Cohesion: 0.10
Nodes (50): AppConfig, _check_provider_token(), _checkpoint(), _choose_camera(), _choose_section(), _configure_apps_api(), _configure_audio(), _configure_camera() (+42 more)
...
Cohesion: 0.11
Nodes (37): Category, _audio_player_ok(), _autostart_ok(), _bot_connectivity(), _camera_ok(), _check(), check_bot_connectivity(), collect_checks() (+29 more)
...
### Community 124 - "test_service.py"
Cohesion: 0.23
Nodes (12): cmd_service(), logs_service(), status_service(), uninstall_service(), Path, _Result, test_cli_service_propagates_failure(), test_install_service_refuses_incomplete_setup_without_systemctl() (+4 more)
...
Cohesion: 0.13
Nodes (34): _bot_token_key(), get_api_token(), get_bot_token(), get_bot_tokens(), get_legacy_bot_token(), import_legacy_env_secrets(), _legacy_bot_tokens_from(), migrate_legacy_bot_token() (+26 more)
...
### Community 192 - "test_cli.py"
Cohesion: 0.14
Nodes (15): cmd_lock(), _interactive_terminal(), main(), _menu_status(), _render_main_menu(), test_all_cli_subcommands_parse_to_callable_handlers(), test_explicit_cli_subcommand_remains_backward_compatible(), test_interactive_no_argument_launch_uses_menu() (+7 more)
...
## Knowledge Gaps
- **579 isolated node(s):** `doctor.sh script`, `BotConfig`, `CommunicationConfig`, `ScreenConfig`, `AppsConfig` (+574 more)
...
- **Why does `AppConfig` connect `AppConfig` to `get_bot_token`, `PersianSpeechManager`, `HttpBotProvider`, `doctor.py`, `config.py`, `test_first_run_regression.py`, `runtime_config.py`, `models.py`, `RuntimeApi`, `InputMonitor`, `test_config.py`, `test_v33_features.py`, `guard.py`, `test_service.py`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `LaptopGuard` connect `LaptopGuard` to `cli.py`, `PersianSpeechManager`, `ChatWindow`, `StopPinStore`, `EventStore`, `SoundDetectionMonitor`, `runtime_config.py`, `RuntimeApi`, `InputMonitor`, `RuntimeStateStore`, `guard.py`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `HttpBotProvider` connect `HttpBotProvider` to `FeatureManager`, `runtime_config.py`, `BotProvider`, `RuntimeApi`?**
...
- **Are the 13 inferred relationships involving `LaptopGuard` (e.g. with `AudioIntercom` and `SecurityChatManager`) actually correct?**
  _`LaptopGuard` has 13 INFERRED edges - model-reasoned connections that need verification._
...
- **What connects `doctor.sh script`, `BotConfig`, `CommunicationConfig` to the rest of the system?**
