...
## Corpus Check
- 102 files · ~34,776 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 905 nodes · 1856 edges · 69 communities (39 shown, 30 thin omitted)
...
### Community 0 - "cli.py"
Cohesion: 0.11
Nodes (34): build_parser(), cmd_arm(), cmd_autostart(), cmd_config(), cmd_disarm(), cmd_doctor(), cmd_events(), cmd_health() (+26 more)
...
Cohesion: 0.06
Nodes (38): Indicator, main(), AudioIntercom, play_audio(), Path, Visible near-live voice intercom.      Bale Bot API transports voice/audio messa, record_audio(), bounded_callback_int() (+30 more)

### Community 4 - "SecurityChatManager"
Cohesion: 0.10
Nodes (24): Direction, DirectionMode, ChatWindow, main(), contains_rtl(), direction_mark(), directional_text(), paragraph_directions() (+16 more)
...
Cohesion: 0.19
Nodes (16): _apply_legacy_env(), _apply_section(), _coerce_like(), ensure_dirs(), get_api_token(), _migrate(), Any, Persist non-secret configuration to config.toml.      Bot/API secrets deliberate (+8 more)

### Community 14 - "AppConfig"
Cohesion: 0.14
Nodes (17): AppConfig, apply_profile(), Apply safe, reversible-ish defaults for a named operating profile.      Profiles, ProfileName, test_enabling_for_next_login_does_not_start_immediately(), test_startup_and_arming_are_separate_defaults(), test_chat_defaults_to_two_minutes_and_allows_visitor_reply(), test_default_warning_sequence_is_five_seconds() (+9 more)
...
Cohesion: 0.23
Nodes (18): BaseException, get_bot_token(), import_legacy_env_secrets(), Import an already-exported legacy bot token into secrets.json once., _ask_valid_token(), _auth_error(), ensure_runtime_configuration(), _interactive() (+10 more)

### Community 20 - "models.py"
Cohesion: 0.11
Nodes (13): ApiConfig, AppsConfig, BotConfig, ChatConfig, CommunicationConfig, MonitorConfig, ScreenConfig, StartupConfig (+5 more)
...
Cohesion: 0.50
Nodes (7): default_api_base(), build_provider(), _choose_camera(), detect_audio_sources(), pair_chat(), Console, run_setup()
...
Cohesion: 0.16
Nodes (6): _append(), _atomic_text(), Path, Visible local chat. Only text entered in its reply box is transmitted., # IMPORTANT: send/store logical Unicode, never visual/reversed text., SecurityChatManager
...
- **Why does `LaptopGuard` connect `LaptopGuard` to `cli.py`, `guard.py`, `BaleApi`, `StopPinStore`, `EventStore`, `test_config_robustness.py`, `RuntimeStateStore`, `RuntimeApi`, `ScreenCaptureManager`, `runtime_config.py`, `launch_warning`?**
  _High betweenness centrality (0.139) - this node is a cross-community bridge._
- **Why does `AppConfig` connect `AppConfig` to `cli.py`, `guard.py`, `config.py`, `RuntimeApi`, `test_v33_features.py`, `runtime_config.py`, `models.py`, `setup_wizard.py`, `test_persian_tts.py`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `FeatureManager` connect `FeatureManager` to `guard.py`?**
