...
10. Run targeted checks first, then the broader verification required below.
11. Review the diff for security/privacy regressions and accidental secrets.
12. Refresh Graphify after material code/docs relationship changes when the tool is
    available, so the next session does not navigate from stale relationships.
13. Report what changed, what was validated, graph freshness status, and any
...
known, then hand its compact map to the specialist agent. The parent agent remains
responsible for reconciling findings and verifying the final patch.
...
tests, and relevant graph freshness are accounted for. Final reports should be
concise and identify any manual target-device validation that remains.
